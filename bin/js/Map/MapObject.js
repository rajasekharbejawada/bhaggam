...
MapObjectSet.prototype.init = function() {
	...
	this.sub = new Subscription(topic, (function(ctx) {
		return function(message) {
			if (!message.destinationName.match("iot-2/type/"+window.config.iot_deviceType+"/id/[A-Za-z0-9]*/evt/telemetry/fmt/json")) { 
			 	return;
			}
			...
			for (var i in data_array) {
				...
				// Traffic simulation: add properties
				ctx.objects[data.id].expectedSpeed = data.expectedSpeed;
...
MapObject.prototype.getPopoverData = function() {
	...
	// Traffic simulation: expected speed
	data.left.content += '<div><b>Expected speed:</b> <span class="value" id="status_expected_speed">'+this.expectedSpeed+'</span></div>';
